const EngagementModels = () => {
  // Array of card data representing different engagement models
  const cardData = [
    {
  ...
  return (
    <div className="p-4 sm:p-8 bg-[#01132E] text-white">
      <div className="text-left mb-8">
        {/* Title for the section "Our Flexible Engagement Models" */}
        <h1
      ...
      {/* Grid layout for the cards (three cards per row on larger screens) */}
      <div className="grid grid-cols-1 md:grid-cols-3 text-center gap-6">
        {/* Mapping through each item in the cardData array */}
        {cardData.map((card, index) => (
          ...
          >
            {/* Pseudo-element to create a background fill effect on hover */}
            <div
            ...
            >
              {card.title} {/* Displaying the title from cardData */}
            </h3>

            {/* Description/content inside the card */}
            <p className="p-4 relative z-10 group-hover:text-black text-base sm:text-base md:text-lg lg:text-lg transition-colors duration-500">
              {card.description} {/* Displaying the description from cardData */}
            </p>
          </div>
        ))}
      </div>
